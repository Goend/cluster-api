// Package locking implements locking functionality for Ansible bootstrap flows.
package locking
...
func NewControlPlaneInitMutex(client client.Client) *ControlPlaneInitMutex {
	return &ControlPlaneInitMutex{client: client}
}

// Lock allows a control plane node to be the first and only node to run kubean cluster initialization.
func (c *ControlPlaneInitMutex) Lock(ctx context.Context, cluster *clusterv1.Cluster, machine *clusterv1.Machine) bool {
	...
	err := c.client.Get(ctx, client.ObjectKey{Namespace: cluster.Namespace, Name: cmName}, sema.ConfigMap)
	switch {
	case apierrors.IsNotFound(err):
		// proceed to create lock
	case err != nil:
		log.Error(err, "Failed to acquire init lock")
		return false
	default:
		info, infoErr := sema.information()
		if infoErr != nil {
			log.Error(infoErr, "Failed to get information about the existing init lock")
			return false
		}
		if info.MachineName == machine.Name {
			return true
		}
		if getErr := c.client.Get(ctx, client.ObjectKey{Namespace: cluster.Namespace, Name: info.MachineName}, &clusterv1.Machine{}); getErr != nil {
			log.Error(getErr, "Failed to get machine holding init lock")
	...
	if err := c.client.Create(ctx, sema.ConfigMap); err != nil {
		switch {
...
func (s semaphore) setInformation(info *information) error {
	bytes, err := json.Marshal(info)
	if err != nil {
		return errors.Wrap(err, "failed to marshal semaphore information")
	}
	s.Data = map[string]string{semaphoreInformationKey: string(bytes)}
	return nil
}
