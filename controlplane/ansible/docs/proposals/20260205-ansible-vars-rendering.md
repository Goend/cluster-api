...
|-------------|------------|----------------------|-----------------|--------------|
| `kube_network_plugin` | Cluster | `spec.extensions.networking.kubeNetworkPlugin` | 用户/ClusterClass | ACP/BA 渲染 vars，决定使用 cilium/flannel 等 |
| `cilium_openstack_project_id` | Cluster | `status.extensions.networking.cilium.projectID` | CAPO | ACP/BA 配置 Cilium 与 OpenStack 集成 |
| `cilium_openstack_default_subnet_id` | Cluster | `status.extensions.networking.cilium.defaultSubnetID` | CAPO | ACP/BA 提供给 VPC CNI |
| `cilium_openstack_security_group_ids` | Cluster | `status.extensions.networking.cilium.securityGroupIDs[]` | CAPO | ACP/BA 配置 Pod/节点安全组 |
| `vpc_cni_webhook_enable` | Cluster | `status.extensions.networking.cilium.webhookEnable` | CAPO | ACP 控制是否部署 webhook |
| `master_virtual_vip` | Cluster | `status.extensions.loadBalancers.controlPlane.vip` | CAPO | ACP Inventory/vars |
| `ingress_virtual_vip` | Cluster | `status.extensions.loadBalancers.ingress.vip` | CAPO | ACP/Harbor |
| `keepalived_interface` | Machine | `spec.extensions.networkInterfaces.keepalived` | 用户/ClusterClass | ACP 设置 keepalived |
| `harbor_addr` | Cluster | `status.extensions.loadBalancers.ingress.vip` | CAPO/业务配置 | ACP 渲染 Harbor 入口 |
| `cloud_master_vip` | Cluster | `status.extensions.openStack.mgmt` | CAPO | ACP/外部访问 |
| `openstack_auth_domain` | Cluster | `status.extensions.openStack.keystone` | CAPO（从 IdentityRef 推导） | ACP vars；凭证通过 SecretRef |
| `openstack_cinder_domain` | Cluster | `status.extensions.openStack.cinder` | CAPO | ACP/存储插件 |
| `openstack_nova_domain` | Cluster | `status.extensions.openStack.nova` | CAPO | ACP/云管配置 |
| `openstack_neutron_domain` | Cluster | `status.extensions.openStack.neutron` | CAPO | ACP/网络插件 |
| `openstack_project_name` | Cluster | `status.extensions.openStack.project` | CAPO | ACP vars |
| `openstack_project_domain_name` | Cluster | `status.extensions.openStack.projectDomain` | CAPO | ACP vars |
| `openstack_region_name` | Cluster | `status.extensions.openStack.region` | CAPO | ACP vars |
| `ntp_server` | Cluster | `status.extensions.platform.ntp.server` | CAPO | ACP hosts/vars |
| `vip_mgmt` | Cluster | `status.extensions.platform.management.vip` | CAPO | ACP/运维脚本 |
| `flannel_interface` | Cluster | `spec.extensions.networkInterfaces.flannel` | 用户/ClusterClass（集群唯一配置） | ACP vars |
| `node_resources.<machine>` | Machine | `status.extensions.nodeResources.reserved` | CAPO | ACP 生成 `node_resources` 字段 |

> 渲染器会直接复用 provider 在 `extensions` 下写入的事实数据，再与 `<cluster>-vars-infra` Config CR 合并。敏感字段（如密码）始终通过 Secret 引用传递；控制面通过 `status.extensions.openStack.appCredential.ref` 读取 CAPO 生成的 `clouds.yaml`，在 cloud-init 写入 `/opt/auth-opts` 与 `/opt/cloud_config`，不在 vars renderer 范围内。
